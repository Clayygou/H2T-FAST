...
class Trainer(BaseTrainer):
    ...
    def do_train_val(self,**kwargs):
        for epoch in range(self.cfg.start_epoch, self.cfg.epochs):
            ...
            current_output = 'Current Prec@1: %.3f\n' % (acc1)
            print(current_output,self.epoch)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, (_input, target) in enumerate(self.val_loader):
                ...
                output, hidden = self.model(_input)
                if best_test:
                    all_hidden.extend(hidden.cpu().numpy()) # 2
